...
This file is the project-local operating contract for AI agents working in `weekly-report-assistant`, a Round 2 dogfood example generated from `agent-dev-os`.
...
## Example-Specific Scope

This example is artifact-only. It demonstrates how a vague product idea becomes a structured artifact chain and one executable task folder.

- Do not implement product source code in this example.
- Do not add a CLI, dependency, database, web UI, agent runtime, service, scheduler, or validation code.
- The first executable task may create only a Markdown commit-summary template artifact.
...
| Path | Purpose |
| --- | --- |
| `product/` | Product requirements, MVP scope, roadmap, user stories, and product-facing artifact templates. |
...
| `src/` | Not present in this example; source code may be added only by a future approved product-code task. |
| `tests/` | Not present in this example; test code may be added only by a future approved product-code task. |
