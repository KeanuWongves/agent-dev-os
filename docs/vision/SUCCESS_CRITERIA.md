...
```text
vision -> product -> architecture -> plan -> task contract -> execution -> review -> validation
```
...
| A task can be delegated to Codex. | `templates/task/TASK_CONTRACT.md` and `templates/codex-prompts/EXECUTE_TASK.md` define bounded execution context. |
...
| Agent roles are clear. | `AGENTS.md` defines product, architecture, planning, execution, review, and validation boundaries. |
...
- An implementation changes files not named or implied by a task contract.
